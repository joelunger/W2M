...
def choose_files():
    ...
    for path in file_paths:
        ...
        file_status_listbox.insert(END, "Wartet...")
...
def convert():
    ...
    for f in sorted_files:
        file_status_listbox.insert(END, f"{os.path.basename(f)} – Wartet...")

    def run_conversion():
        successful_files = []
        for i, file in enumerate(sorted_files):
            try:
                file_status_listbox.delete(i)
                file_status_listbox.insert(i, f"{os.path.basename(file)} – Wird konvertiert...")
                ...
                file_status_listbox.delete(i)
                file_status_listbox.insert(i, f"{os.path.basename(file)} – Fertig!")
                root.update_idletasks()
                successful_files.append(f"{title}.mp3")
            except Exception as e:
                file_status_listbox.delete(i)
                file_status_listbox.insert(i, f"{os.path.basename(file)} – Fehler: {str(e)}")
                continue
